...
m_magnetic_field_i = m_magnetic_field_temp_i[init:(init+N),:].copy()
...
Bdifferenceb = np.zeros((N,3))
Bdifferencei = np.zeros((N,3))
...
v_w0_BIB = -np.array([0.001, np.sqrt(G*M_EARTH/(r)**3)+0.001, 0.001]) #input is in radian per second
...
for  i in range(0,N-1):
	...
	'''
	#if math.fmod(i,20) == 0:
	Bdifferenceb[i,:] = v_magnetic_field_b_c - v_magnetic_field_b_p
	Bdifferencei[i,:] = m_magnetic_field_i[i+1,1:4] - m_magnetic_field_i[i,1:4]
...
os.mkdir('SSO-Identity-no-dist-rate-0.001-k-1-vlong')
os.chdir('SSO-Identity-no-dist-rate-0.001-k-1-vlong')
np.savetxt('Bdifferenceb.csv',Bdifferenceb, delimiter=",")
np.savetxt('Bdifferencei.csv',Bdifferencei, delimiter=",")
